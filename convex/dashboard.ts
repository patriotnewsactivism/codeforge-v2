...
export const getDashboard = query({
  ...
  handler: async (ctx, args) => {
    ...
    const missionStats = {
      ...
      last7Days: missions.filter((m: any) => m._creationTime > now - week)
        .length,
      successRate: missions.length
        ? Math.round(
            (missions.filter((m: any) => m.status === "completed").length /
              missions.length) *
              100,
          )
        : 0,
    ...
    const deployStats = {
      ...
      deployed: deployments.filter(
        d => d.status === "deployed" || d.status === "canary",
      ).length,
      awaitingApproval: deployments.filter(d => d.status === "awaiting_human")
        .length,
    ...
    const debateStats = {
      ...
      avgConfidence: debates.length
        ? Math.round(
            debates.reduce((s, d) => s + d.confidence, 0) / debates.length,
          )
        : 0,
      avgDurationMs: debates.length
        ? Math.round(
            debates.reduce((s, d) => s + d.durationMs, 0) / debates.length,
          )
        : 0,
    };
    ...
    const learningStats = {
      ...
      mutationsPending: mutations.filter(m => m.status === "pending_review")
        .length,
      totalLessonsLearned: reflections.reduce(
        (s, r) => s + r.lessonsLearned.length,
        0,
      ),
    };
    ...
    const incidentStats = {
      ...
      autofixed: incidents.filter(
        i => i.status === "pr_opened" || i.status === "resolved",
      ).length,
...
export const getCostBreakdown = query({
  ...
  handler: async (ctx, args) => {
    ...
    const allCosts = await ctx.db
      ...
      .filter(q => q.gte(q.field("_creationTime"), since))
      .take(500);
    const costs = allCosts.filter(
      (c: any) => c.buildSessionId && sessionIds.has(c.buildSessionId),
    );
