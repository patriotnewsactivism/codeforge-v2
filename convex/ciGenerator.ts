...
export const generateSmokeTests = action({
  ...
  handler: async (ctx, args) => {
    ...
    const codeFiles = files.filter((f: any) => !f.isDirectory && (f.path.endsWith(".tsx") || f.path.endsWith(".ts")));

    let appRoutes = "No routes found.";
    const appTsx = codeFiles.find((f: any) => f.path.includes("src/App.tsx") || f.path.includes("src/main.tsx"));
    if (appTsx) {
...
Output ONLY the raw file content, inside a markdown code block.`;
    ...
    const { text } = await callAIWithFallback([{ role: "user", content: prompt }], {
      model,
    });
