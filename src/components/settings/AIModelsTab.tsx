...
const SWARM_PROFILES = [
  {
    ...
    description:
      "DeepSeek R1 handles complex plans, Gemini 2.5 Flash writes high-speed code, and Claude 3.5 Haiku catches bugs during reviews. Insane quality/price ratio.",
    reasoning: 5,
    speed: 4,
  ...
  },
  {
    ...
    description:
      "Uses Qwen Coder for planning, Kimi K2 for code generation, and Gemini Flash for reviews. Spawns 10+ agents for literally pennies.",
    reasoning: 3,
    speed: 4,
    priceLevel: 1,
  ...
  },
  {
    ...
    description:
      "Claude 3.5 Sonnet architecting and reviewing, OpenAI GPT-4.5 doing the heavy lifting of code generation. Extremely robust but pricier.",
    reasoning: 5,
    speed: 3,
    priceLevel: 5,
  ...
  },
  {
    ...
    description:
      "o3-mini reasons about the high-level architecture, DeepSeek R1 executes code logic with deep chain-of-thought, and Claude Haiku reviews.",
    reasoning: 5,
    speed: 3,
    priceLevel: 3,
  ...
  },
  {
    ...
    description:
      "Gemini 2.5 Flash does fast planning and debugging, Kimi K2 generates code instantly, and GPT-4o Mini performs blazing-fast reviews.",
    reasoning: 3,
    speed: 5,
    priceLevel: 2,
...
export function AIModelsTab() {
  ...
  const handleSelectProfile = async (id: string) => {
    ...
    try {
      ...
      toast.success(
        `Swarm Profile switched to ${SWARM_PROFILES.find(p => p.id === id)?.name} ✓`,
      );
    } catch (err) {
      toast.error(
  ...
  const filteredModels = MODELS.filter(m => {
    ...
    if (filter === "premium")
      return matchesSearch && (m.costLevel === "$$$" || m.costLevel === "$$$$");
            ...
            >
              ...
              <div>
                <div className="flex items-center gap-2.5">
                  <div
                    className={`p-2 rounded-lg ${isSelected ? "bg-primary/10 text-primary" : "bg-zinc-900 text-zinc-400 group-hover:text-zinc-200"}`}
                  >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h3 className="text-md font-semibold text-white flex items-center gap-2">
              <Layers className="h-4 w-4 text-zinc-400" /> Model Catalog &
              Capabilities
            </h3>
            <p className="text-xs text-zinc-400 mt-0.5">
              Exact pricing per 1M tokens (input / output) for the connected
              models.
            </p>
          </div>
        ...
        <div className="border border-zinc-800/80 rounded-xl bg-zinc-950/10 overflow-hidden divide-y divide-zinc-900">
          ...
          {filteredModels.length === 0 ? (
            ...
            filteredModels.map(m => (
              <div
                key={m.id}
                className="grid grid-cols-12 text-xs items-center px-4 py-3 hover:bg-zinc-900/10 transition-colors"
              >
                ...
                <div className="col-span-2 flex justify-center items-center gap-1 px-4">
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map(b => (
                      <div
                        key={b}
                        className={`h-2.5 w-1.5 rounded-sm ${b <= m.reason ? "bg-primary" : "bg-zinc-800"}`}
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="col-span-2 flex justify-center items-center gap-1 px-4">
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map(b => (
                      <div
                        key={b}
                        className={`h-2.5 w-1.5 rounded-sm ${b <= m.quality ? "bg-violet-500" : "bg-zinc-800"}`}
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="col-span-1 flex justify-center items-center gap-1">
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map(b => (
                      <div
                        key={b}
                        className={`h-2.5 w-1.5 rounded-sm ${b <= m.speed ? "bg-cyan-500" : "bg-zinc-800"}`}
                      />
                    ))}
                  </div>
                </div>
