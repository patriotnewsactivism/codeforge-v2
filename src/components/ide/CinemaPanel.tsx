...
import {
  Play, Pause, SkipBack, SkipForward, Film,
  Zap, MessageSquare, Shield, Swords, Brain, CheckCircle, XCircle, GitBranch
} from "lucide-react";
...
function FrameCard({ frame, isActive }: { frame: CinemaFrame; isActive: boolean }) {
  let payload: Record<string, unknown> = {};
  try { payload = JSON.parse(frame.payload); } catch {}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-semibold text-foreground truncate">{frame.agentName}</span>
        <Badge variant="outline" className="text-[9px] px-1 py-0 shrink-0">{frame.frameType}</Badge>
        {frame.agentRole && (
          <span className="text-muted-foreground text-[9px] shrink-0">{frame.agentRole}</span>
        )}
        {frame.durationMs && (
          <span className="ml-auto text-muted-foreground text-[9px] shrink-0">{frame.durationMs}ms</span>
        )}
        {frame.success === false && <XCircle className="h-3 w-3 text-red-400 shrink-0" />}
        {frame.success === true && <CheckCircle className="h-3 w-3 text-green-400 shrink-0" />}
      </div>

      {/* Show relevant payload fields */}
      {payload.content && (
        <p className="text-muted-foreground leading-relaxed line-clamp-2">{String(payload.content)}</p>
      )}
      {payload.tool && (
      ...
      {payload.output && !payload.content && (
        <p className="text-green-300/80 line-clamp-2">{String(payload.output).slice(0, 120)}</p>
      )}
        ...
        {!frames || frames.length === 0 ? (
          ...
          <div className="flex flex-col gap-1">
            {frames.map((frame, i) => (
              <div
                key={frame._id}
                ref={(el) => { if (el) frameRefs.current.set(i, el); }}
        ...
        <div className="shrink-0 border-t border-border px-3 py-2 bg-[oklch(0.10_0.02_260)]">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              variant="ghost"
              className="h-7 w-7"
              onClick={() => { setCurrentIndex(totalFrames - 1); setIsPlaying(false); }}
            >
