...
export function ChatPanel({
  ...
  const handleSend = async () => {
    ...
    try {
      // Check if user is using @codeforge mention
      const cleanMsg = msg.replace(/@codeforge\s*/gi, "").trim();
      const contextMsg = currentFileContent
        ? cleanMsg
        : cleanMsg;
    ...
    <div className="h-full flex flex-col bg-[oklch(0.11_0.02_260)]">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border">
        ...
        <div className="relative">
          <button
            type="button"
            className="flex items-center gap-1.5 px-2 py-1 text-xs rounded bg-[oklch(0.18_0.02_260)] hover:bg-[oklch(0.22_0.02_260)] transition-colors"
            onClick={() => setModelDropdownOpen(!modelDropdownOpen)}
          >
            <span>{currentModelConfig.icon}</span>
            <span className={currentModelConfig.color}>
              {currentModelConfig.name}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
          {modelDropdownOpen && (
            <div className="absolute right-0 top-full mt-1 z-50 bg-[oklch(0.16_0.02_260)] border border-border rounded-md shadow-lg py-1 min-w-[180px]">
              {MODELS.map((model) => (
                <button
                  key={model.id}
                  type="button"
                  className={cn(
                    "flex items-center gap-2 w-full px-3 py-1.5 text-xs hover:bg-[oklch(0.22_0.02_260)] transition-colors",
      ...
      {session && session.totalCost > 0 && (
        <div className="flex items-center gap-2 px-3 py-1 border-b border-border bg-[oklch(0.12_0.02_260)] text-xs text-muted-foreground">
      ...
      {currentFileName && (
        <div className="px-3 py-1 text-[10px] text-muted-foreground border-t border-border flex items-center gap-1">
      ...
      <div className="px-3 py-2 border-t border-border">
        <div className="flex gap-2">
          <textarea
            ...
            placeholder="Ask @codeforge anything..."
            value={input}
