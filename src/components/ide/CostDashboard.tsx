...
import { Activity, BarChart3, Cpu, DollarSign, TrendingUp } from "lucide-react";
...
export function CostDashboard({
  ...
  for (const entry of entries) {
    ...
    byModel.set(key, {
      ...
      tokens:
        prev.tokens + (entry.inputTokens || 0) + (entry.outputTokens || 0),
      calls: prev.calls + 1,
    });
  }

  const modelEntries = [...byModel.entries()].sort(
            ...
            <div key={model}>
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-1.5">
                  <span
                    className={cn("w-2 h-2 rounded-full", getModelColor(model))}
                  />
      ...
      <div className="px-3 pb-3 flex-1">
        ...
        <div className="space-y-0.5">
          {entries
            .slice(-20)
            .reverse()
            .map((entry: NonNullable<typeof entries>[number]) => (
              <div
                key={entry._id}
