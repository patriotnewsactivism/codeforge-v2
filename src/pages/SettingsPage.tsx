...
import {
  ChevronRight, Github, Key, Loader2, Moon,
  Palette, Save, Sun, User, Zap, CheckCircle2, Eye, EyeOff,
} from "lucide-react";
    ...
    >
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage your account, appearance, and integrations</p>
        </div>
          ...
          {activeTab === "account" && (
            <Card className="border-slate-800 bg-slate-900">
              ...
              <CardContent className="space-y-2">
                ...
                >
                  <div>
                    <p className="font-medium text-sm text-slate-200">Change password</p>
                    <p className="text-sm text-slate-500">Update your password via email code</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between rounded-lg border border-slate-800 p-4 hover:bg-slate-800/50 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="size-10 rounded-full bg-slate-800 flex items-center justify-center">
                        {theme === "light" ? <Moon className="size-5 text-slate-300" /> : <Sun className="size-5 text-slate-300" />}
                      </div>
                      <div>
                        <Label htmlFor="dark-mode" className="font-medium text-slate-200">Dark mode</Label>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    with <code className="text-xs bg-slate-800 px-1 rounded">repo</code> scope.
                  </p>
                  <div className="flex gap-2">
                    ...
                    >
                      {savingToken ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}
                      Save
                    </Button>
                  </div>
                </div>
      ...
      <Dialog open={changePasswordOpen} onOpenChange={setChangePasswordOpen}>
        <DialogContent className="bg-slate-900 border-slate-800">
          <DialogHeader>
            <DialogTitle className="text-slate-100">Change Password</DialogTitle>
          ...
          {passwordStep === "request" ? (
            <form onSubmit={handleRequestPasswordReset}>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setChangePasswordOpen(false)} className="border-slate-700">
                  Cancel
                </Button>
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input name="code" placeholder="Enter code from email" className="bg-slate-800 border-slate-700 text-slate-200" />
              </div>
              <div className="space-y-2">
                <Label className="text-slate-300">New password</Label>
                <Input type="password" name="newPassword" placeholder="••••••••" className="bg-slate-800 border-slate-700 text-slate-200" />
              </div>
              {error && <p className="text-sm text-red-400 bg-red-500/10 rounded-lg px-3 py-2">{error}</p>}
              {success && <p className="text-sm text-emerald-400 bg-emerald-500/10 rounded-lg px-3 py-2">{success}</p>}
              <DialogFooter>
                <Button variant="outline" onClick={() => setChangePasswordOpen(false)} className="border-slate-700">Cancel</Button>
                <Button type="submit" disabled={loading}>
                  {loading && <Loader2 className="size-4 animate-spin" />}
                  Change Password
                </Button>
      ...
      <Dialog open={deleteAccountOpen} onOpenChange={setDeleteAccountOpen}>
        <DialogContent className="bg-slate-900 border-slate-800">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              This is permanent. All your projects, files, and sessions will be deleted.
            </DialogDescription>
          </DialogHeader>
          {error && <p className="text-sm text-red-400 bg-red-500/10 rounded-lg px-3 py-2">{error}</p>}
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteAccountOpen(false)} className="border-slate-700">
              Cancel
            </Button>
            <Button
              variant="destructive"
