...
runTest("Agent Generation", async helper => {
  ...
  if (await newProjectBtn.isVisible().catch(() => false)) {
    ...
    const nameInput = page.locator("input[placeholder*='Project Name'], input[name='name']").first();
  ...
  if (!url.includes("/project/")) {
    ...
    if (await projectLink.isVisible().catch(() => false)) {
      ...
      throw new Error("Could not create or find a project to test agent generation");
    }
  }
  ...
  const chatInput = page.locator("textarea[placeholder*='Message'], textarea[placeholder*='Ask']").first();
  ...
  const hasResponse = await page.locator(".prose, .markdown-body, [data-agent-message='true']").count() > 0;
